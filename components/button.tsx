...
export default function Button({ m, toHome, ...props }: ButtonProps) {
    ...
    return (
        <motion.a
            ...
            className={
                (m.disabled
                    ? 'bg-white/30 text-bmw/50 cursor-default'
                    : 'bg-white/50' + ' hover:bg-ymw/70 text-bmw hover:text-black cursor-pointer') +
            ...
            onClick={m.name === 'Home' ? toHome : m.inner && !m.disabled ? () => router.push(m.href || path) : () => {}}
            // disabled={m.disabled}
        ...
        >
            {m.fa ? (
                ...
                <Image
                    src={m.icon}
                    className={'h-6 md:h-8 lg:h-10 ' + (m.disabled ? 'opacity-20' : hover && 'grayscale')}
                    alt={m.name}
                    height={100}
                    width={100}
                    style={{
            ...
            <span className='font-CS font-bold text-xs sm:text-base md:text-lg lg:text-xl text-center'>{m.name}</span>
        </motion.a>
    );
}
