...
export default function Club() {
  ...
  useEffect(() => setSelectedType(clubType), [])
  useEffect(() => setSelectedAct(clubAct), [])
  useEffect(() => {
  ...
  return (
    <>
        <title>MWIT Club | MWIT Open House 2024</title>
      ...
      <main className='w-full bg-phd/20'>
        ...
        <div className='flex flex-wrap relative overflow-y-hidden max-w-6xl mx-auto px-6 justify-center gap-4 pt-4 pb-10'>
          {clubData
            ...
            .map((c, i) => (
              <Link className='rounded-xl bg-white hover:bg-ycbg transition-colors duration-300 shadow-lg max-w-xs overflow-hidden' href={'/club/' + c.name.replaceAll(' ', '_')} key={i}>
                <Image
                    src={
                        process.env.CDN_URL +
                        '/img/club/cover/' +
